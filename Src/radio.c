...
void radio_configure_tx(void)
{
  ...
  // address (0x2B, 0x01, 0x02, 0x03, 0x04, 0x00)
  write_buffer[0] = (RADIO_W_REGISTER | RADIO_TX_ADDR);
  write_buffer[1] = 1;
  write_buffer[2] = 2;
  write_buffer[3] = 3;
  write_buffer[4] = 4;
  ...
  write_buffer[0] = (RADIO_W_REGISTER | RADIO_ADDR_P0);
  write_buffer[1] = 1;
  write_buffer[2] = 2;
  write_buffer[3] = 3;
  write_buffer[4] = 4;
  ...
  write_buffer[0] = (RADIO_W_REGISTER | RADIO_CONFIG);
  write_buffer[1] = (RADIO_PWR_UP | RADIO_MASK_RX_DR | RADIO_MASK_TX_DS | RADIO_MASK_MAX_RT);
  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_0, GPIO_PIN_RESET);
  HAL_SPI_Transmit(&hspi1, write_buffer, 2, 1000);
  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_0, GPIO_PIN_SET);
}
...
void radio_send(uint8_t *data, size_t length)
{
  ...
  for (size_t currentFrame = 0; currentFrame < length; currentFrame += 32)
  {
    ...
    // copy to buffer and set TX to first byte
    write_buffer[0] = RADIO_W_TX_PAYLOAD;
    memcpy(&write_buffer[1], &data[currentFrame], currentFrameSize);

    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_0, GPIO_PIN_RESET);
    HAL_SPI_Transmit(&hspi1, write_buffer, currentFrameSize+1, 1000);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_0, GPIO_PIN_SET);
  }
}
